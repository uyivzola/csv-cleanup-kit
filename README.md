...
`drop_exact` compares **all normalized fields**, retains the first occurrence, and operates **within each file**. It does not deduplicate across files or assume matching amounts alone prove duplicates. `quarantine_keys` quarantines **every** record in a repeated-key group, including its first member; empty keys are also quarantined. Key comparisons use normalized numeric values and untouched ID/text values.
